...
PR #130ではPronounce + WAVを1つのhost Undo単位に載せ、YMM4標準Undo/Redo commandからも往復できることを確認済みです。残る主なLocal Assist課題は、Undo managerのproduct-grade public取得経路・interactive preview/audio cache・Effect disable/removeです。Voice Review側のv0 identityは、Guidではなくsession ref + fingerprint再解決を採用します。
